...
import { register, checkId } from '@/api/authApi'
...
watch(password, (val) => {
  ...
  if (passwordConfirm.value) {
    const match = val === passwordConfirm.value
    passwordConfirmMatch.value = match
    passwordConfirmMessage.value = match ? '비밀번호가 일치합니다.' : '비밀번호가 일치하지 않습니다.'
  }
})
